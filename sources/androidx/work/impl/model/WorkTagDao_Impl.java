package androidx.work.impl.model;

import android.database.Cursor;
import androidx.room.EntityInsertionAdapter;
import androidx.room.RoomDatabase;
import androidx.room.RoomSQLiteQuery;
import androidx.room.util.DBUtil;
import androidx.sqlite.p005db.SupportSQLiteStatement;
import java.util.ArrayList;
import java.util.List;

public final class WorkTagDao_Impl implements WorkTagDao {

    /* renamed from: a */
    private final RoomDatabase f5300a;

    /* renamed from: b */
    private final EntityInsertionAdapter f5301b;

    /* renamed from: androidx.work.impl.model.WorkTagDao_Impl$a */
    class C1123a extends EntityInsertionAdapter<WorkTag> {
        C1123a(WorkTagDao_Impl workTagDao_Impl, RoomDatabase roomDatabase) {
            super(roomDatabase);
        }

        /* renamed from: c */
        public void bind(SupportSQLiteStatement supportSQLiteStatement, WorkTag workTag) {
            String str = workTag.tag;
            if (str == null) {
                supportSQLiteStatement.bindNull(1);
            } else {
                supportSQLiteStatement.bindString(1, str);
            }
            String str2 = workTag.workSpecId;
            if (str2 == null) {
                supportSQLiteStatement.bindNull(2);
            } else {
                supportSQLiteStatement.bindString(2, str2);
            }
        }

        public String createQuery() {
            return "INSERT OR IGNORE INTO `WorkTag`(`tag`,`work_spec_id`) VALUES (?,?)";
        }
    }

    public WorkTagDao_Impl(RoomDatabase roomDatabase) {
        this.f5300a = roomDatabase;
        this.f5301b = new C1123a(this, roomDatabase);
    }

    public List<String> getTagsForWorkSpecId(String str) {
        RoomSQLiteQuery acquire = RoomSQLiteQuery.acquire("SELECT DISTINCT tag FROM worktag WHERE work_spec_id=?", 1);
        if (str == null) {
            acquire.bindNull(1);
        } else {
            acquire.bindString(1, str);
        }
        this.f5300a.assertNotSuspendingTransaction();
        Cursor query = DBUtil.query(this.f5300a, acquire, false);
        try {
            ArrayList arrayList = new ArrayList(query.getCount());
            while (query.moveToNext()) {
                arrayList.add(query.getString(0));
            }
            return arrayList;
        } finally {
            query.close();
            acquire.release();
        }
    }

    public List<String> getWorkSpecIdsWithTag(String str) {
        RoomSQLiteQuery acquire = RoomSQLiteQuery.acquire("SELECT work_spec_id FROM worktag WHERE tag=?", 1);
        if (str == null) {
            acquire.bindNull(1);
        } else {
            acquire.bindString(1, str);
        }
        this.f5300a.assertNotSuspendingTransaction();
        Cursor query = DBUtil.query(this.f5300a, acquire, false);
        try {
            ArrayList arrayList = new ArrayList(query.getCount());
            while (query.moveToNext()) {
                arrayList.add(query.getString(0));
            }
            return arrayList;
        } finally {
            query.close();
            acquire.release();
        }
    }

    public void insert(WorkTag workTag) {
        this.f5300a.assertNotSuspendingTransaction();
        this.f5300a.beginTransaction();
        try {
            this.f5301b.insert(workTag);
            this.f5300a.setTransactionSuccessful();
        } finally {
            this.f5300a.endTransaction();
        }
    }
}
